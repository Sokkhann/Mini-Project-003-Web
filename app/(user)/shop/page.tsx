...
export default function DashBoard() {
  ...
  const[productdetail,setProductDetail]=useState<CartProductType>()
...
const ProductDetail=(product:CartProductType)=>{
...
const handleFilter = (event: any) => {
  ...
  const columns:TableColumn<CartProductType>[] = [
    ...
    {
      name: 'Image',
      selector: (row):JSX.Element|any => <img className='w-[80px] h-[70px]' src={row.image} width={500} height={500}  alt={row.title}/>,
    },
       ...
       <Modal show={openModal} onClose={() => setOpenModal(false)}>
        ...
        <Modal.Body>
          <div className="space-y-6">
            <img src={productdetail?.image||placeHolderImage} width={500} height={500} alt={productdetail?.title||"Untitle"} className="w-full h-96 object-fit" />
